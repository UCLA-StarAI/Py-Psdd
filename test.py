...
def dfs(psdd, asgn, d, l):
    if d == l:
        cache = {}
        res = algo.compute_probability(psdd, asgn, cache)
        return res
    res = 0.0
    if d < 10:
...
if __name__ == '__main__':
    ...
    sdd = util.sdd_from_file('./workspace/B.sdd', './workspace/B.vtree')
    cache_lit, cache_idx = {}, {}
    sdd = algo.normalize(sdd, util.vtree_from_file('./workspace/B.vtree'), cache_lit, cache_idx)
    sdd.node_count = algo.re_index(sdd)
    cache = {}
    psdd = util.sdd_to_psdd(sdd, cache)
    data_set = data.DataSet(train_data_file="./workspace/B.train")
    algo.set_data(psdd, data_set.train)
    algo.compute_parameter(psdd)
    util.psdd_to_file(psdd, './workspace/B.psdd')
